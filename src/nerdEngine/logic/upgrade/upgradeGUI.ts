...
export class UpgradeGUI {
    ...
    GetPrice(price: number | Wallet | Price): Price {
    ...
    GetTweakValue(tweakOrSearchMethod: number | Tweak = 0, whatToReturnIfNotBought: Float = new Float(0)): Float {
    ...
    GetTweakDataValue(tweakOrSearchMethod: number | Tweak = 0, gameDataID: number): Float {
        const tweak = this.GetTweak(tweakOrSearchMethod);
        return tweak.TargetList[gameDataID].Value;
    }

    //todo there can be some refactoring
    GetTweakDataValueOnLevel(tweakOrSearchMethod: number | Tweak = 0, gameDataID: number, relativeLevel: RawFloat | null): Float {
        const tweak = this.GetTweak(tweakOrSearchMethod)
